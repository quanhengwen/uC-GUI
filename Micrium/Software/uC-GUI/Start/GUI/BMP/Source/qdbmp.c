...
BMP* BMP_ReadFile( BFILE* f )
{
	...
	if ( bmp->Header.BitsPerPixel == 8 )
	{
		...
		if ( BFREAD( bmp->Palette, BMP_PALETTE_SIZE, f ) != BMP_PALETTE_SIZE )
		{
			BMP_LAST_ERROR_CODE = BMP_FILE_INVALID;
			BFCLOSE( f );
			BFREE( bmp->Palette );
	...
	/* Read image data */
	if ( BFREAD( bmp->Data, bmp->Header.ImageDataSize, f ) != bmp->Header.ImageDataSize )
	{
		BMP_LAST_ERROR_CODE = BMP_FILE_INVALID;
		BFCLOSE( f );
		BFREE( bmp->Data );
...
**************************************************************/
void BMP_WriteFile( BMP* bmp, BFILE* f )
{
	...
	if ( bmp->Palette )
	{
		if ( BFWRITE( bmp->Palette, BMP_PALETTE_SIZE, f ) != BMP_PALETTE_SIZE )
		{
			BMP_LAST_ERROR_CODE = BMP_IO_ERROR;
			BFCLOSE( f );
			return;
	...
	/* Write data */
	if ( BFWRITE( bmp->Data, bmp->Header.ImageDataSize, f ) != bmp->Header.ImageDataSize )
	{
		BMP_LAST_ERROR_CODE = BMP_IO_ERROR;
		BFCLOSE( f );
		return;
