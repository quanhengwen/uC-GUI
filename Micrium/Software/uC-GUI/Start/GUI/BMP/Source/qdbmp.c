...
int    ReadHeader   ( BMP* bmp, BFILE* f );
int    WriteHeader  ( BMP* bmp, BFILE* f );

int    ReadUINT     ( UINT* x, BFILE* f );
int    ReadUSHORT   ( USHORT *x, BFILE* f );

int    WriteUINT    ( UINT x, BFILE* f );
int    WriteUSHORT  ( USHORT x, BFILE* f );
...
BMP* BMP_Create( UINT width, UINT height, USHORT depth )
{
  ...
  {
    bmp->Palette = (UCHAR*) BCALLOC( BMP_PALETTE_SIZE, sizeof( UCHAR ) );
    if ( bmp->Palette == NULL )
    {
      BMP_LAST_ERROR_CODE = BMP_OUT_OF_MEMORY;
      BFREE( bmp );
  ...
  /* Allocate pixels */
  bmp->Data = (UCHAR*) BCALLOC( bmp->Header.ImageDataSize, sizeof( UCHAR ) );
  if ( bmp->Data == NULL )
  {
    BMP_LAST_ERROR_CODE = BMP_OUT_OF_MEMORY;
    BFREE( bmp->Palette );
...
BMP* BMP_ReadFile( const char* filename )
{
  ...
  if ( bmp->Header.BitsPerPixel == 8 )
  {
    bmp->Palette = (UCHAR*) BMALLOC( BMP_PALETTE_SIZE * sizeof( UCHAR ) );
    if ( bmp->Palette == NULL )
    {
      BMP_LAST_ERROR_CODE = BMP_OUT_OF_MEMORY;
      BFCLOSE( f );
    ...
    if ( BFREAD( bmp->Palette, sizeof( UCHAR ), BMP_PALETTE_SIZE, f ) != BMP_PALETTE_SIZE )
    {
      BMP_LAST_ERROR_CODE = BMP_FILE_INVALID;
      BFCLOSE( f );
      BFREE( bmp->Palette );
  ...
  bmp->Data = (UCHAR*) BMALLOC( bmp->Header.ImageDataSize );
  if ( bmp->Data == NULL )
  {
    BMP_LAST_ERROR_CODE = BMP_OUT_OF_MEMORY;
    BFCLOSE( f );
  ...
  /* Read image data */
  if ( BFREAD( bmp->Data, sizeof( UCHAR ), bmp->Header.ImageDataSize, f ) != bmp->Header.ImageDataSize )
  {
    BMP_LAST_ERROR_CODE = BMP_FILE_INVALID;
    BFCLOSE( f );
    BFREE( bmp->Data );
...
void BMP_WriteFile( BMP* bmp, const char* filename )
{
  ...
  if ( bmp->Palette )
  {
    if ( BFWRITE( bmp->Palette, sizeof( UCHAR ), BMP_PALETTE_SIZE, f ) != BMP_PALETTE_SIZE )
    {
      BMP_LAST_ERROR_CODE = BMP_IO_ERROR;
      BFCLOSE( f );
      return;
  ...
  /* Write data */
  if ( BFWRITE( bmp->Data, sizeof( UCHAR ), bmp->Header.ImageDataSize, f ) != bmp->Header.ImageDataSize )
  {
    BMP_LAST_ERROR_CODE = BMP_IO_ERROR;
    BFCLOSE( f );
    return;
...
int  WriteUINT( UINT x, BFILE* f )
{
  ...
  return ( f && BFWRITE( little, 4, 1, f ) == 1 );
}
...
int  WriteUSHORT( USHORT x, BFILE* f )
{
  ...
  return ( f && BFWRITE( little, 2, 1, f ) == 1 );
}
