...
static void _DrawWindowsDirectoryBitmaps(void) {
  ...
  sprintf(acMask, "%s\\Training\\BitmapConverter\\*.jpg", "C:\\Keil_v5\\ARM\\Segger\\emWin\\Doc");
  hFind = FindFirstFile(acMask, &Context);
  if (hFind != INVALID_HANDLE_VALUE) {
    do {
      sprintf(acFile, "%s\\Training\\BitmapConverter\\%s", "C:\\Keil_v5\\ARM\\Segger\\emWin\\Doc", Context.cFileName);
